...
Transformer backbone layers used by the REVE encoder. Kept semantically
identical to the released code, with the local module layout only.
"""
import torch
...
class Attention(nn.Module):
    ...
    def __init__(self, dim, heads=8, head_dim=64, use_flash=True):
        ...
        # flash-attn only executes on CUDA with fp16/bf16 tensors; keep a
        # classical fallback for CPU execution and float32 evaluation passes.
        self.classical_attend: nn.Module | None = (
            ClassicalAttention(self.heads, use_sdpa=True) if use_flash else None
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        attend = self.attend
        if self.use_flash and self.classical_attend is not None:
            if not x.is_cuda or x.dtype not in (torch.float16, torch.bfloat16):
                attend = self.classical_attend
        x = self.norm(x)
        qkv = self.to_qkv(x)
        out = attend(qkv)
